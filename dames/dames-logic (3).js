...
// L'API lidraughts exporte les rafles en SEGMENTS séparés ("27x18" "18x9"…)
// — vu dans GameApiV2.scala qui émet pdnMoves brut sans pdnMovesConcat.
// Cette fonction refait exactement leur fusion : deux prises consécutives
// dont la fin de l'une est le début de la suivante = la même rafle qui
// continue. Le résultat "27x18x9" garde les cases de PASSAGE, ce qui lève
// au passage la plupart des ambiguïtés de chemin.
function dMergeCaptureSegments(tokens) {
  const merged = [];
  for (const tok of tokens) {
    const prev = merged[merged.length - 1];
    if (prev && prev.includes('x') && tok.includes('x')) {
      const prevEnd = prev.slice(prev.lastIndexOf('x') + 1).split(/[x-]/)[0];
      const tokStart = tok.slice(0, tok.indexOf('x'));
      if (prevEnd === tokStart) {
        merged[merged.length - 1] = prev + tok.slice(tok.indexOf('x'));
        continue;
      }
    }
    merged.push(tok);
...
function dReplayMoves(tokens) {
  const clean = dMergeCaptureSegments(tokens.map((t) => t.trim()).filter(Boolean));
  const warnings = [];
...
if (typeof module !== 'undefined') {
  module.exports = { dMergeCaptureSegments, sqToXY, xyToSq, dNewState, dLegalMoves, dApplyMove, dReplayMoves, dToFen, dFenToState, dToScanFen, dMoveNotation, dGameStatus };
}
