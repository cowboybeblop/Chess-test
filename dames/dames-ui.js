...
const BRILLIANT_META = { label: 'Brillant', symbol: '★', color: '#1baaa6' };
...
function renderResults(records, headers, username) {
  ...
  const bw = records.filter((r) => r.brilliant && r.color === 'w').length;
  const bb = records.filter((r) => r.brilliant && r.color === 'b').length;
  const brillRow = `<div class="ct-row${bw === 0 && bb === 0 ? ' zero' : ''}">
      <span class="ct-label">${BRILLIANT_META.label} <span style="color:var(--text-dim);font-size:10px;">(bonus)</span></span>
      <span class="ct-count" style="color:${BRILLIANT_META.color}">${bw}</span>
      <span class="dico-pin" style="background:${BRILLIANT_META.color}">★</span>
      <span class="ct-count" style="color:${BRILLIANT_META.color}">${bb}</span></div>`;
  $('classTable').innerHTML = brillRow + ORDER.map((key) => {
...
function moveCell(rec, plyIndex) {
  ...
  if (rec.brilliant) {
    ...
    star.className = 'badge'; star.style.color = BRILLIANT_META.color; star.textContent = BRILLIANT_META.symbol;
...
/* ---- Navigation + damier ---- */
// Fait défiler UNIQUEMENT la liste interne vers l'élément actif, sans
// jamais faire bouger la page elle-même (scrollIntoView fait défiler tous
// les ancêtres, y compris la page -> c'était le bug du "ça descend tout
// seul à chaque coup suivant").
function scrollListToActive(list, el) {
  if (!list || !el) return;
...
function goToPly(ply) {
  ...
  if (explore) { explore = null; } // naviguer dans la partie quitte l'exploration
  ...
  scrollListToActive($('moveList'), document.querySelector('.move-cell.active'));
  updateExploreButton();
}
...
/* =========================================================
   Exploration de la ligne moteur ("pourquoi ce coup ?")
   Depuis la position courante, on rejoue la variante calculée par
   Scan coup par coup, avec éval en direct à chaque étape.
   "⟲ Retour à la partie" ramène à la position d'où l'on est parti
   (PAS au début de la partie).
   ========================================================= */
let explore = null; // { basePly, fens:[], moves:[hubStr], notations:[], idx, evalToken }

function updateExploreButton() {
  const hasLine = currentRecords && currentPly < currentRecords.length &&
    currentRecords[currentPly] && (currentRecords[currentPly].bestPv || []).length > 0 || canLiveExplore();
  $('btnExplore').classList.toggle('hidden', !!explore || !currentRecords || currentPly >= currentRecords.length);
  $('exploreActive').classList.toggle('hidden', !explore);
}
function canLiveExplore() { return !!engine; }

$('btnExplore').addEventListener('click', async () => {
  if (!currentRecords || currentPly >= currentRecords.length) return;
  const rec = currentRecords[currentPly]; // le coup à venir depuis la position affichée
  let pv = rec.bestPv || [];
  if (!pv.length) {
    // ancienne analyse sans PV stockée : on la recalcule en direct
    $('btnExplore').disabled = true;
    $('btnExplore').textContent = '🔎 Calcul de la ligne…';
    try {
      const st = dFenToState(currentPlies[currentPly].fen);
      const r = await getEngine().analyze(dToScanFen(st), { depth: parseInt($('depthSelect').value, 10) });
      pv = r.pv || [];
    } catch (e) { /* tant pis */ }
    $('btnExplore').disabled = false;
    $('btnExplore').textContent = '🔎 Jouer la ligne moteur';
    if (!pv.length) { setState('error', 'Pas de ligne moteur disponible ici.'); return; }
  }
  // préparer toutes les positions de la ligne en rejouant les coups hub
  const fens = [currentPlies[currentPly].fen];
  const notations = [];
  let st = dFenToState(fens[0]);
  for (const hubStr of pv) {
    const hub = parseHubMove(hubStr);
    const legal = dLegalMoves(st);
    const mv = legal.find((m) => sameMove(hub, m)) || legal.find((m) => m.from === hub.from && m.to === hub.to);
    if (!mv) break; // sécurité : on s'arrête si un coup de la PV ne matche pas
    st = dApplyMove(st, mv);
    fens.push(dToFen(st));
    notations.push(dMoveNotation(mv));
  }
  if (fens.length < 2) { setState('error', 'Ligne moteur inexploitable ici.'); return; }
  explore = { basePly: currentPly, fens, notations, idx: 1, evalToken: 0 };
  stepExplore(0); // affiche l'état après le 1er coup (idx=1)
});

function exitExplore(backToBase) {
  if (!explore) return;
  const base = explore.basePly;
  explore = null;
  if (backToBase) goToPly(base);
  updateExploreButton();
}
$('btnExpReset').addEventListener('click', () => exitExplore(true));
$('btnExpNext').addEventListener('click', () => stepExplore(1));
$('btnExpPrev').addEventListener('click', () => {
  if (explore && explore.idx <= 1) { exitExplore(true); return; }
  stepExplore(-1);
});

async function stepExplore(delta) {
  if (!explore) return;
  explore.idx = Math.max(1, Math.min(explore.fens.length - 1, explore.idx + delta));
  const fen = explore.fens[explore.idx];
  renderPosition(fen);
  // flèche du PROCHAIN coup de la ligne, s'il existe
  const svg = $('arrowSvg');
  svg.innerHTML = '';
  updateExploreButton();
  // fil d'Ariane : coups joués de la ligne, le courant surligné
  $('exploreTrail').innerHTML = '<b>Ligne moteur :</b> ' + explore.notations.map((n, k) =>
    k === explore.idx - 1 ? `<span class="et-cur">${n}</span>` : (k < explore.idx ? n : `<span style="opacity:.45">${n}</span>`)
  ).join(' · ');
  $('moveInfo').innerHTML = `<span class="mi-start">Exploration — coup ${explore.idx}/${explore.fens.length - 1} de la variante · <b style="color:var(--teal)">⟲</b> pour revenir à la partie</span>`;
  // éval en direct de la position explorée (rapide, annulable)
  const token = ++explore.evalToken;
  try {
    const st = dFenToState(fen);
    if (dLegalMoves(st).length) {
      const r = await getEngine().analyze(dToScanFen(st), { depth: 9 });
      if (explore && token === explore.evalToken) {
        const cpWhite = st.turn === 'w' ? r.scoreCp : -r.scoreCp;
        const winInWhite = r.winIn != null ? (st.turn === 'w' ? r.winIn : -r.winIn) : null;
        updateEvalBar({ evalCpWhite: cpWhite, winInWhite });
        // flèche du coup suivant selon le moteur, depuis la position explorée
        if (explore.idx < explore.fens.length - 1 || r.bestMove) {
          const nxt = parseHubMove(r.bestMove);
          if (nxt) { renderExploreArrow(nxt); }
        }
      }
    }
  } catch (e) { /* silencieux */ }
}
function renderExploreArrow(hub) {
  const svg = $('arrowSvg');
  ...
  svg.innerHTML = '';
  const c = (sq) => { const { x, y } = sqToXY(sq); return [x * cell + cell / 2, y * cell + cell / 2]; };
  const [x1, y1] = c(hub.from), [x2, y2] = c(hub.to);
  const pl = document.createElementNS('http://www.w3.org/2000/svg', 'line');
  pl.setAttribute('x1', x1); pl.setAttribute('y1', y1); pl.setAttribute('x2', x2); pl.setAttribute('y2', y2);
  pl.setAttribute('stroke', '#2fb3ab'); pl.setAttribute('stroke-width', cell * 0.12);
  pl.setAttribute('stroke-linecap', 'round'); pl.setAttribute('stroke-dasharray', '5,4'); pl.setAttribute('opacity', 0.6);
  svg.appendChild(pl);
}
...
function updateMoveInfo() {
  ...
  if (rec.brilliant) {
    html += `<span class="mi-chip" style="color:${BRILLIANT_META.color};border-color:${BRILLIANT_META.color}55;background:${BRILLIANT_META.color}18">★ Brillant</span>`;
  }
  html += `<span class="mi-eval">${formatEval(rec)}</span>`;
...
function renderPosition(fen) {
  ...
  const used = new Set();
  // PASSE 1 : d'abord ancrer TOUTES les pièces qui n'ont pas bougé (même
  // type, même case). PASSE 2 : ensuite seulement, attribuer les disques
  // restants aux pièces déplacées. Les deux passes doivent être séparées :
  // fusionnées en une seule boucle, une pièce déplacée peut "voler" le
  // disque d'une pièce immobile et déclencher une réorganisation en
  // cascade de tout le damier (l'ancien bug).
  wanted.forEach((wp) => {
    const hit = existing.find((el) => !used.has(el) && el.dataset.key === wp.key && el.dataset.sq == wp.sq);
...
function renderArrows() {
  ...
  if (currentRecords && currentPly > 0) {
    const rec = currentRecords[currentPly - 1];
    const meta = rec.brilliant ? BRILLIANT_META : CLASS_META[rec.cls];
