...
function renderResults(records, headers, username) {
  ...
  // Précision de partie : méthode Lichess (pondération par volatilité +
  // moyenne harmonique), calculée sur la partie entière — voir gameAccuracy.
  ...
  const eloTextFor = (arr, acc) => arr.length >= MIN_MOVES_FOR_ELO
    ? `≈ ${estimatedEloFromAccuracy(acc)}`
  ...
  summaryEl.innerHTML = `
    <div class="side-summary"><b>${headers.White || 'Blancs'}</b>${whiteIsUser ? ' (toi)' : ''}<br>
      ...
      Elo officiel : ${headers.WhiteElo || '?'} · Elo estimé sur cette partie ${eloTextFor(wReal, accW)}</div>
    <div class="side-summary"><b>${headers.Black || 'Noirs'}</b>${blackIsUser ? ' (toi)' : ''}<br>
      ...
      Elo officiel : ${headers.BlackElo || '?'} · Elo estimé sur cette partie ${eloTextFor(bReal, accB)}</div>
