...
const btnPlayLine10 = $('btnPlayLine10');
const btnPlayLine20 = $('btnPlayLine20');
const extendedPanel = $('extendedPanel');
const extendedInfo = $('extendedInfo');
const btnExtPrev = $('btnExtPrev');
const btnExtNext = $('btnExtNext');
const btnExtExit = $('btnExtExit');
...
function showTab(which) {
  const tabs = { analyze: [tabAnalyze, viewAnalyze], history: [tabHistory, viewHistory], lab: [$('tabLab'), $('viewLab')] };
...
if ($('tabLab')) $('tabLab').addEventListener('click', () => showTab('lab'));
...
async function runAnalysis(pgn, username) {
  ...
  try {
    ...
    }, { whiteElo: parseInt(headers.WhiteElo, 10) || 0, blackElo: parseInt(headers.BlackElo, 10) || 0 });
  } catch (e) {
...
function renderResults(records, headers, username) {
  ...
  // Tableau bilan façon Chess.com : nombre de coups de chaque type par camp.
  // Calculé à la volée depuis les records -> marche aussi pour les analyses
  // rechargées depuis l'historique, y compris celles d'anciennes versions.
  const counts = { w: {}, b: {} };
  ...
  $('classTable').innerHTML = CLASS_ORDER.map((key) => {
    const m = CLASS_META[key];
    const cw = counts.w[key] || 0, cb = counts.b[key] || 0;
    const dimRow = (cw === 0 && cb === 0) ? ' zero' : '';
    return `<div class="ct-row${dimRow}">
      ...
      <span class="ct-count" style="color:${m.color}">${cb}</span>
    </div>`;
  }).join('');
...
function goToPly(ply) {
  ...
  scrollListToActive(moveListEl, moveListEl.querySelector('.move-cell.active'));
}
...
/* =========================================================
   3bis. Jouer la suite du moteur (10/20 coups) — appels moteur
   séquentiels en direct depuis la position affichée, indépendant
   de toute ligne stockée pendant l'analyse initiale.
   ========================================================= */
let extendedLine = null; // { positions:[{fen, sanFr}], index, returnPly }
let extendedRunning = false;

async function playOutEngineLine(numMoves) {
  if (!currentPlies || extendedRunning) return;
  extendedRunning = true;
  btnPlayLine10.disabled = true; btnPlayLine20.disabled = true;
  const eng = getEngine();
  const depth = Math.min(parseInt(depthSelect.value, 10) || 14, 14); // reste réactif sur 20 coups d'affilée
  let state = fenToState(currentPlies[currentPly].fen);
  const positions = [{ fen: currentPlies[currentPly].fen, sanFr: null }];
  try {
    for (let i = 0; i < numMoves; i++) {
      setState('analyzing', `Suite du moteur… (${i + 1}/${numMoves})`);
      const fen = toFEN(state, 1);
      const result = await eng.analyze(fen, { depth, multipv: 1, limitStrength: false });
      if (!result.bestMoveUci) break;
      const from = algToXY(result.bestMoveUci.slice(0, 2));
      const to = algToXY(result.bestMoveUci.slice(2, 4));
      const moves = legalMoves(state, from.x, from.y);
      const move = moves.find((m) => m.to.x === to.x && m.to.y === to.y);
      if (!move) break;
      const promo = result.bestMoveUci.length > 4 ? result.bestMoveUci[4] : 'q';
    ...
    btnPlayLine10.disabled = false; btnPlayLine20.disabled = false;
    return;
  }
  setState('done', `Suite du moteur : ${positions.length - 1} coup(s) calculé(s).`);
  extendedRunning = false;
  btnPlayLine10.disabled = false; btnPlayLine20.disabled = false;
...
function renderExtendedLine() {
  ...
  extendedInfo.textContent = index === 0
    ? `Suite du moteur — position de départ (${positions.length - 1} coup(s) disponible(s))`
...
btnPlayLine10.addEventListener('click', () => playOutEngineLine(10));
btnPlayLine20.addEventListener('click', () => playOutEngineLine(20));
...
function updateMoveInfo() {
  ...
  if (currentPly === 0) {
    moveInfoEl.innerHTML = '<span class="mi-start">Position de départ</span><span class="mi-note mi-note-empty"></span>';
    return;
  }
  ...
  html += `<span class="mi-note${rec.note ? '' : ' mi-note-empty'}" style="border-left-color:${meta.color}">${rec.note || ''}</span>`;
