...
/* =========================================================
   Filet de sécurité : toute erreur JS non attrapée s'affiche
   à l'écran (pas de console sur mobile)
   ========================================================= */
window.addEventListener('error', (e) => {
  setState('error', 'Erreur JS : ' + (e.message || 'inconnue') + (e.filename ? ` (${e.filename.split('/').pop()}:${e.lineno})` : ''));
});
window.addEventListener('unhandledrejection', (e) => {
  const msg = e.reason && e.reason.message ? e.reason.message : String(e.reason);
  setState('error', 'Erreur async : ' + msg);
});
