...
function renderResults(records, headers, username) {
  ...
  const accW = avgAccuracy(byColor.w), accB = avgAccuracy(byColor.b);
  const acplW = avgEpLoss(byColor.w), acplB = avgEpLoss(byColor.b);
  ...
  summaryEl.innerHTML = `
    ...
    <div class="acc-note">Classification calée sur le tableau officiel Chess.com "Classification V2" (points attendus, 9 fév. 2026) — sauf calibration par rating, propriétaire et non reproduite ici. Précision et Elo estimé restent des approximations.</div>
  `;
